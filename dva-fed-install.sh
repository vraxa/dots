...
if [[ "$DRY_RUN" == true ]]; then
    dry_run_msg "Would install DaVinci Resolve dependencies:"
    ...
    dry_run_msg "Would prompt to download and install DaVinci Resolve from Blackmagic website"
else
...
# Ly is available in Fedora repos or via Copr
if [[ "$DRY_RUN" == true ]]; then
    dry_run_msg "Would install ly display manager"
    dry_run_msg "Would disable current display manager (gdm/sddm/lightdm)"
    dry_run_msg "Would enable ly.service"
else
    # Try to install ly from repos first, then Copr
    if ! sudo dnf install -y ly; then
        log "Ly not in official repos, trying Copr..."
        sudo dnf copr enable -y atim/ly || warning "Failed to enable ly Copr repo"
        sudo dnf install -y ly || {
            warning "Ly installation failed. You may need to build from source:"
            warning "  git clone --recurse-submodules https://github.com/fairyglade/ly"
            warning "  cd ly && make && sudo make install installsystemd"
        }
    fi
    
    if command -v ly &> /dev/null || [[ -f /usr/bin/ly ]]; then
        success "Ly installed successfully"
        
        # Disable current display managers
        log "Disabling current display manager..."
        ...
        success "Ly is now your display manager (will take effect after reboot)"
    else
        warning "Ly binary not found after installation attempt"
    fi
fi
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    echo -e "  ${CYAN}•${NC} Ly display manager (replaces GDM)"
    echo -e "  ${CYAN}•${NC} Thunar configured with dark theme + set as default"
    echo -e "  ${CYAN}•${NC} DaVinci Resolve dependencies + GPU drivers"
    ...
    echo -e "  ${GREEN}✓${NC} Ly display manager (GDM disabled)"
    ...
    echo "  2. Ly will greet you at the login screen"
    echo "  3. Select 'Hyprland' and log in!"
    echo
    log "Key bindings:"
